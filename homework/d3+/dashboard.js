// list of countries and their country_codes
var country_codes = [
    ["af", "AFG", "Afghanistan"],
...
// lists for the data will still be processed therefre pre
pre_countries = []
pre_ranks = []
pre_indexes = []
pre_expectations = []
...
// loads all the data
d3.csv("hpi_data.csv", function(error, data) {
  data.forEach(function(d) {
    // push the variables into lists
    pre_countries.push(d.country)
    // console.log(pre_countries)
    pre_ranks.push(parseInt(d.rank))
    pre_indexes.push(parseFloat(d.index))
    pre_expectations.push(parseFloat(d.exp))
    pre_well_beings.push(parseFloat(d.being))
    pre_foot_prints.push(parseFloat(d.foot))
  })

  // length of country_codes and length of countries
  lenCodes = country_codes.length
  ...
  for(var i = 0; i < lenCountries; i++){
    for (var j = 0; j < lenCodes; j++){
      if (pre_countries[i] == country_codes[j][2]){
        country_ids.push(country_codes[j][1])
        ranks.push(pre_ranks[i])
        indexes.push(pre_indexes[i])
        expectations.push(pre_expectations[i])
        well_beings.push(pre_well_beings[i])
        foot_prints.push(pre_foot_prints[i])
      }
    }
  }
